@extends('admin.layouts.master')
@section('title')PopUp Manage  @endsection
@section('body')
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            <div class="page-title-right">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                      <li class="breadcrumb-item"><a href="{{route('popups.create')}}">PopUp Module</a></li>
                      <li class="breadcrumb-item active" aria-current="page">PopUp Manage</li>
                    </ol>
                  </nav>
                 </div>
            <h2 class="page-title">PopUp Module</h2>
        </div>
    </div>
</div>
    <div class="row">
        <div class="col-lg-12">
            <!--create-->
            @can('popups.create')
            <div class="my-1">
                <a  href="{{route('popups.create')}}">
                    <button class="btn btn-info"   data-bs-toggle="tooltip" data-bs-placement="top" title="Size Create"><i class="fa-solid fa-square-plus"></i> Add </button>
                </a>
            </div>
            @endcan
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">All Size Information</h4>
                    <p class="text-muted font-14">{{Session::get('success')}}</p>
                    <div class="table-responsive">
                    <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                        <thead>
                        <tr>
                            <th>SL NO</th>
                            <th>Image </th>
                            <th>Image Link </th>
                            <th>Status </th>
                            @if(auth()->user()->can('popups.edit') || auth()->user()->can('popups.destroy') )
                            <th>Action</th>
                            @endif
                        </tr>
                        </thead>
                        <tbody>
                            @if ($popUps->count()>0)
                            @foreach($popUps as $popUp)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>
                                        @if ($popUp->image)
                                        <img src="{{ asset($popUp->image) }}" style="width: 30px ; height:30px">
                                        @else
                                        <img src="{{asset('admin')}}/assets/not-found/notfound.jpeg" alt="NO image" style="width: 30px; height:30px">
                                        @endif

                                    </td>
                                    <td>{{$popUp->image_link}}</td>
                                    <td>
                                        @if ($popUp->status==1)
                                        <span class="badge p-1 bg-success">Published</span>
                                        @else
                                        <span class="badge p-1 bg-danger">Unpublished</span>
                                        @endif
                                    </td>

                                   @if(auth()->user()->can('popups.edit') || auth()->user()->can('popups.destroy') )
                                    <td>
                                        <!--edit-->
                                       @can('popups.edit')
                                        <a href="{{route('popups.edit',$popUp->id)}}"
                                             class="btn btn-success btn-sm"
                                             data-bs-toggle="tooltip" data-bs-placement="top" title="Size Edit">
                                            <i class="ri-edit-box-fill"></i>
                                        </a>

                                        @if ($popUp->status==1)
                                            <a href="{{route('popups.show',$popUp->id)}}"
                                               class="btn btn-info btn-sm"
                                               data-bs-toggle="tooltip" data-bs-placement="top" title="Unpublished">
                                                <i class="fa-solid fa-lock"></i>
                                            </a>

                                        @endif
                                        @if ($popUp->status==2)
                                            <a href="{{route('popups.show',$popUp->id)}}"
                                               class="btn btn-warning btn-sm"
                                               data-bs-toggle="tooltip" data-bs-placement="top" title="Published">
                                                <i class="fa-solid fa-lock-open py-"></i>
                                            </a>
                                        @endif
                                       @endcan
                                        <!--Destroy-->
                                        {{-- @can('popups.destroy')
                                        <form action="{{route('popups.destroy',$popUp->id)}}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger btn-sm"
                                            data-bs-toggle="tooltip" data-bs-placement="top" title="Size Delete"
                                             onclick="return confirm('Are you sure you want to delete this teacher?');">
                                                <i class="ri-chat-delete-fill"></i>
                                            </button>
                                        </form>
                                        @endcan --}}
                                    </td>
                                   @endif
                                </tr>
                            @endforeach
                            @else
                            <tr>
                                <td colspan="6" class="text-center">Data not Available </td>
                            </tr>
                            @endif
                        </tbody>
                    </table>
                </div>
                </div>  <!-- end card-body -->
            </div>  <!-- end card -->
        </div>  <!-- end col -->
    </div>
@endsection
